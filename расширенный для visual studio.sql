...
--calc summ
	...
	outer apply 
			(--присоединяем обещания
				...
				where d.id = dp.parent_id and dp.prom_date between @d1 and @d2
...
--последний мусорный контакт
	left join( select cl.r_debt_id, cl.dt
				from i_collect.dbo.contact_log as cl
				where cl.result in (8,10,206,211,306,307,308,312,317,319,
									325,326,703,724,730,731,736,811,812,813,
									814,815,120159,120161,120162,120163,120164,
									320159,320164,320266)					
	...
	left join(
				...
				from i_collect.dbo.contact_log as cl
				where cl.result in (		1,2,4,5,11,12,14,15,16,201,202,204,
										207,208,210,212,213,706,707,712,714,715,
										717,718,719,720,721,723,726,729,737,738,
										739,740,816,817,818,819,820,821,824,825,
										826,838,839,840,842,861,862,863,865,866,
										867,868,870,874,875,877,880,120154,120155,
										120156,120183,120186,120187,120188,120189,
										120190,120191,120193,120194,120197,120198,
										120199,120200,120201,120202,120205,120206,
										120207,120214,120215,120216,120217,120218,
										120221,120222,120223,120224,120225,120226,
										120227,120228,120231,120233,120234,120235,
										120237,120239,120240,120241,320157,320161,
										320272,320273,320274,320280,320283,320284,
										320287,320297,320300,320301,320304,320310,
										320311,320313,320316,320317,320318,320319,
										320320,320322
								)					
					and cl.id in(	select max(id)
									from i_collect.dbo.contact_log
			...
			(
				select perspect.r_debt_id,
					...
					count(case
						when perspect.result in (
									1,2,4,5,11,12,14,15,16,201,202,204,
									207,208,210,212,213,706,707,712,714,715,
									717,718,719,720,721,723,726,729,737,738,
									739,740,816,817,818,819,820,821,824,825,
									826,838,839,840,842,861,862,863,865,866,
									...
									320287,320297,320300,320301,320304,320310,
									320311,320313,320316,320317,320318,320319,
									320320,320322,320712,320607,320616,320609,
									320713,320614,320617,320610,320714,320635,
									320704,320637,320715,320640,320705,320710)
						then perspect.id
				...
				where
					...
					and perspect.result in (1,2,3,4,5,9,11,12,13,14,15,16,30,201,202,203,204,
											...
											320610,	320652,320648,320649,320650,320651,320714,
											320635,320704,320636,320637,320715,320640,
											320705,320641,320711,320710,320642,320706,
											320707,320708,320709)
					and d.id = perspect.r_debt_id
				group by
					perspect.r_debt_id			
			)perspect
where b.id in (@bank) and ( case when d.status in (6,7,8,10)	then 0	else 1	end) in (@status)
group by p.name,	d.debt_sum,c.calc_date,pr.prom_date,b.id,d.typ,la.start_date,p.sign_date,p.end_date,	d.id,
