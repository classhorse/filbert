USE [wh_data]
GO
/****** Object:  StoredProcedure [dbo].[Filbert_HOROVOD]    Script Date: 04.08.2016 13:14:17 ******/
/*	1 - сортировка
	2 - удаление закрытых дел
	3 - удаление закрепленных дел
	4 - удаление рабочих телефонов
	5 - удаление долгов с обещаниями
*/
SET ANSI_NULLS ON
GO
...
set nocount on;
BEGIN
	if OBJECT_ID('#___closed') is not null
		drop table #___closed
	;if OBJECT_ID('#___phone') is not null
		drop table #___phone
	;if OBJECT_ID('#___prom') is not null
		drop table #___prom
	;if OBJECT_ID('#___fixed') is not null
		drop table #___fixed
	;if OBJECT_ID('#___sort') is not null
		drop table #___sort
	...
	;with
		...
		,__closed as
				...
				from
					...
					inner join _______closed_mx on _______closed_mx.id = dbl.id
				)	
	...
	select * into #___phone from _______phone
	...
	;with
		...
		,__fixed as
				...
				from
					...
					inner join _______fix f on wt.id = f.id
				where
					wt.r_user_id not in (1604, -1)
				)
	...
	;with 			
		...
		select * into #___sort from __sort
...
--<district_1>
	if @n = 1 --конъюнктура
		...
		from
			#tmp_campaign_1 tc1
			left join (select id, dt from #___sort)cl on tc1.[ID] = cl.id
		order by
			cl.dt asc
		...
		;delete from [INFINITY2].[Cx_Work].[public].[Table_5000081023]
			...
			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон1] = null
				where [Телефон1] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон2] = null
				where [Телефон2] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон3] = null
				where [Телефон3] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон4] = null
				where [Телефон4] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон5] = null
				where [Телефон5] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон6] = null
				where [Телефон6] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон7] = null
				where [Телефон7] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон8] = null
				where [Телефон8] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон9] = null
				where [Телефон9] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон10] = null
				where [Телефон10] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон11] = null
				where [Телефон11] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон12] = null
				where [Телефон12] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон13] = null
				where [Телефон13] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон14] = null
				where [Телефон14] in (select num from #___phone)

			;UPDATE [INFINITY2].[Cx_Work].[public].[Table_5000081023]
				set [Телефон15] = null
				where [Телефон15] in (select num from #___phone)
...
--------------------------------
		...
		--<вставляем отсортированные данные>
			...
			from
				...
				left join (select id, dt from #___sort)cl on tc11.[ID] = cl.id
			order by
				cl.dt asc
...
--<district_2>
		...
		from
			#tmp_campaign_2 tc2
			left join (select id, dt from #___sort)cl on tc2.[ID] = cl.id
		order by
			cl.dt asc
		...
		where [ID] in (select id from #___closed)
			or [ID] in (select id from #___fixed)
		...
		if OBJECT_ID ('#tmp_campaign_21') is not null
			drop table #tmp_campaign_21
				...
				from
					...
					left join (select id, dt from #___sort)cl on tc21.[ID] = cl.id
				order by
					cl.dt asc
...
--<district_3>
		...
		if OBJECT_ID ('#tmp_campaign_3') is not null
			...
			from
				#tmp_campaign_3 tc3
				left join (select id, dt from #___sort)cl on tc3.[ID] = cl.id
			order by
				cl.dt asc
		...
		if OBJECT_ID ('#tmp_campaign_31') is not null
			drop table #tmp_campaign_31
				...
				from
					...
					left join (select id, dt from #___sort)cl on tc31.[ID] = cl.id
				order by
					cl.dt asc
...
--<district_4>
		...
		if OBJECT_ID ('#tmp_campaign_4') is not null
			...
			from
				#tmp_campaign_4 tc4
				left join (select id, dt from #___sort)cl on tc4.[ID] = cl.id
			order by
				cl.dt asc
		...
		if OBJECT_ID ('#tmp_campaign_41') is not null
			 drop table #tmp_campaign_41
				...
				from
					...
					left join (select id, dt from #___sort)cl on tc41.[ID] = cl.id
				order by
					cl.dt asc
...
--<district_5>
		...
		if OBJECT_ID('#tmp_campaign_5') is not null
			...
			from
				#tmp_campaign_5 tc5
				left join (select id, dt from #___sort)cl on tc5.[ID] = cl.id
			order by
				cl.dt asc
		...
		if OBJECT_ID ('#tmp_campaign_51') is not null
			drop table #tmp_campaign_51
				...
				from
					...
					left join (select id, dt from #___sort)cl on tc51.[ID] = cl.id
				order by
					cl.dt asc
...
--<district_6>
		...
		if OBJECT_ID ('#tmp_campaign_6') is not null
			...
			from
				#tmp_campaign_6 tc6
				left join (select id, dt from #___sort)cl on tc6.[ID] = cl.id
			order by
				cl.dt asc
		...
		if OBJECT_ID ('#tmp_campaign_61') is not null
			drop table #tmp_campaign_61
				...
				from
					...
					left join (select id, dt from #___sort)cl on tc61.[ID] = cl.id
				order by
					cl.dt asc
...
--<district_7>
		...
		if OBJECT_ID('#tmp_campaign_7') is not null
			...
			from
				#tmp_campaign_7 tc7
				left join (select id, dt from #___sort)cl on tc7.[ID] = cl.id
			order by
				cl.dt asc
		...
		if OBJECT_ID('#tmp_campaign_71') is not null
			drop table #tmp_campaign_71
				...
				from
					...
					left join (select id, dt from #___sort)cl on tc71.[ID] = cl.id
				order by
					cl.dt asc
...
--<district_8>
		...
		if OBJECT_ID('#tmp_campaign_8') is not null
			...
			from
				#tmp_campaign_8 tc8
				left join (select id, dt from #___sort)cl on tc8.[ID] = cl.id
			order by
				cl.dt asc
		...
		if OBJECT_ID('#tmp_campaign_81') is not null
			drop table #tmp_campaign_81
				...
				from
					...
					left join (select id, dt from #___sort)cl on tc81.[ID] = cl.id
				order by
					cl.dt asc
...
--<district_9>
		...
		if OBJECT_ID('#tmp_campaign_9') is not null
			...
			from
				#tmp_campaign_9 tc9
				left join (select id, dt from #___sort)cl on tc9.[ID] = cl.id
			order by
				cl.dt asc
		...
		if OBJECT_ID('#tmp_campaign_91') is not null
			drop table #tmp_campaign_91
				...
				from
					...
					left join (select id, dt from #___sort)cl on tc91.[ID] = cl.id
				order by
					cl.dt asc
...
--<district_10>
		...
		if OBJECT_ID('#tmp_campaign_10') is not null
			...
			from
				...
				left join (select id, dt from #___sort)cl on tc10.[ID] = cl.id
			order by
				cl.dt asc
		...
		if OBJECT_ID('#tmp_campaign_101') is not null
			drop table #tmp_campaign_101
				...
				from
					...
					left join (select id, dt from #___sort)cl on tc101.[ID] = cl.id
				order by
					cl.dt asc
